CircoTax_DESeq2<- function(input_phyloseq, # REQUIRED
                           contrast, # REQUIRED
  ...
  library("phyloseq")
  library("ggh4x")
  if(lfc_shrink_method=="apeglm"){
    library("apeglm")
  }
  if(lfc_shrink_method=="ashr"){
    library("ashr")
  }
  ...
  Warnings_ls<-list(
    ...
    B = "\n * The argument B (DESeq2 BaseMean arbitrary threshold) has been set as 'B=50' by default",
    ...
    lfc = "\n * The argument lfc (log fold change threshold) has been set as 'lfc=1' by default",
    lfc_shrink_method= "\n * The argument lfc_shrink_method (shrinkage method of lfc) has been set as 'none' by default\n(the possible alternatives are 'apeglm' and 'ashr')",
  ...
  ####### Checking eventual invalid lfc shrinkage methods
  
  if(length(lfc_shrink_method)>1 | length(lfc_shrink_method)==0 | !(lfc_shrink_method %in% c("none","apeglm","ashr"))){
    stop("\n\n Please choose one lfc shrinkage method among 'none' (default), 'apeglm' and 'ashr' \n\n")
    Sys.sleep(1)
  }
  
  if(lfc_shrink_method=="apeglm" & length( unique(sample_data(data)[[contrast[1]]]) )>2 ){
    cat("\n Warning: due to the multiple levels present in the target factor, re-setting the reference level may be required to use the apeglm shrinkage (see DESeq2 tutorial)! \n\n")
    Sys.sleep(2)
  }
  ...
  if(! all( is.numeric(B),is.numeric(p_val),is.numeric(W),is.numeric(H),is.numeric(lfc)) ){
  ...
  if(! all( length(B)==1,length(p_val)==1, length(W)==1, length(H)==1, length(lfc)==1) ){
  ...
  if( is.character(sample_data(data)[[contrast[1]]])){
    sample_data(data)[[contrast[1]]]<-as.factor(sample_data(data)[[contrast[1]]])   # it would be converted by DESeq2 anyways BUT with a lot of annoying warnings!
  ...
  ######################## STARTING DA WITH DESEQ2 ###########################
  
  ####### Preparing the objects for DESeq2 loop
  ...
  for( t in tax_in ){
    ...
    DEseq_data<-phyloseq_to_deseq2(d, design = formula(design) )
    DE<-DESeq(DEseq_data)
    if(lfc_shrink_method=="none"){
      res<-results(DE, contrast= contrast) # In DESeq2 itself the contrast is specified as follows contrast=c("Condition","Healthy","RCC")
    }
    if(lfc_shrink_method=="apeglm"){
      coef_input<-resultsNames(DE)[ grepl(pattern = contrast[1], resultsNames(DE)) & grepl(pattern = contrast[2], resultsNames(DE))]
      res<-lfcShrink(DE, coef= coef_input, type="apeglm" ) 
    }
    if(lfc_shrink_method=="ashr"){
      res<-lfcShrink(DE, contrast= contrast, type="ashr" )
    }
    res$padj<- p.adjust( res$pvalue , method = p_adjustment )   # NB: is "BH" by default, but can be overwritten if otherwise specified 
    ...
    if( length(res$log2FoldChange) >0 ){ # if there are results...
      ...
      Table_DE<- suppressWarnings(psmelt(target))
      ...
      cat("\nNo results for the",t,"level\n")
      Sys.sleep(1)
    }
  }
  
  if( length(Res_tot$ASV) > 1 & auto_save==TRUE ){ # if there are results
    write.csv(Res_tot, file=paste0(save_path,"DESeq2_results.csv"), row.names = F)
  }
  
  
  ################## PLOTTING THE RESULTS (AN UNIQUE "IF" THAT STARTS FROM THERE) #####################
  
  if( length(Res_tot$ASV) > 1){ # if there are results
    ...
    if(format_image=="png" & plot_boxplot==TRUE & auto_save==TRUE){
      ggsave(filename = paste0(save_path,"DESeq2_Boxplot.png"), device=format_image, width = W, height = H, units = "in", dpi=300)
    }
    if(format_image=="pdf" & plot_boxplot==TRUE & auto_save==TRUE){
      ggsave(filename = paste0(save_path,"DESeq2_Boxplot.pdf"), device=format_image, width = W, height = H)
    }
    ...
    if(plot_circo==TRUE){   # preparing the object with the plot in it
      CircoTax2(Res_tot2,title="",ramp=c("orange","white","blue"),tax_col=8:12,fc_col=2,sort=sort_circo) +
  ...
  if(length(Res_tot$ASV) > 1 & auto_save==FALSE){
    Results_DESeq2<<-Res_tot2
    Boxplot_DESeq2<<-Table_tot2
    Circoplot_DESeq2 <<- CircoTax2(Res_tot2,title="",ramp=c("orange","white","blue"),tax_col=8:12,fc_col=2,sort=sort_circo) +
      labs(title=paste(contrast[3],"vs",contrast[2],"\n DA taxa log2foldchange\n"))
    Sys.sleep(1)
    cat("\nDue to auto_save==FALSE, the following objects have been saved in your environment:\nResults_DESeq2 Boxplot_DESeq2 Circoplot_DESeq2\n ")
    Sys.sleep(1)
  }
  ...
  Settings<-cbind.data.frame(statistical_design=design, # with cbind it's possible to specify the col names directly
                             BaseMean_threshold=B,
                             p_value_threshold=p_val,
                             p_value_adj_method=p_adjustment,
                             log2FC_theshold=lfc,
                             lfc_shrinkage_method= lfc_shrink_method,
  ...
  if(auto_save==TRUE){
    write.table(x = t(Settings), 
                file=paste0(save_path,"DESeq2_Circoplot_Settings.csv"),
